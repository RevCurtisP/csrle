/*
 *  cistopbm: Convert images in the CompuServe RLE format to Portable BitMap
...
int main(int argc, char **argv)
{
	...
	for (n = 1; n < argc; n++)
	{
		...
		if (inoptions)
		{
			...
			if (pm_keymatch(argv[n], "-p", 2) ||
			    pm_keymatch(argv[n], "-P", 2) ||
			    pm_keymatch(argv[n], "--plain", 7))
			{
				plain = 1;
				continue;
			}
	...
	input  = pm_openr(inpname);
	output = pm_openw(outname);
...
/* If character read was EOF, end of file was reached and magic number not
 * found. */
		if (c[2] == EOF)
		{
...
/* Other non-printing characters are ignored; some files contain a BEL */
...
/* See if we decoded the right number of pixels */
	...
	pbm_writepbm(output, bits, width, height, plain);
	pm_close(output);
	pm_close(input);
	return 0;	
}
